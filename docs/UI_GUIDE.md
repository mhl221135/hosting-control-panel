...
| **Save profiles** | Validates and saves the reusable Low, Medium, and High PHP-FPM worker, idle, request-timeout, recycling, and per-worker memory-estimate defaults. Existing pools are not changed. |
...
| **Apply to existing pools** | Opens a preview of every affected pool and each changed value. Select individual matching pools; custom/drifted pools are excluded. Unselected matching pools retain their settings and become custom/drifted after the profile definition changes. Type `APPLY` to confirm the immutable reviewed snapshot. Backs up pool-presets.json, pools.conf, and sites.map, validates with `php-fpm -t`, reloads PHP-FPM through the controlled action, verifies every pool port, and restores changed files automatically if validation or reload fails. |
| **Estimated memory per worker** | A validated planning estimate (32-4096 MB) per profile. It drives the capacity summary and is never written into PHP-FPM pool configuration. Changing only this value never makes pools drift, triggers a reload, or appears in the apply preview. |
| **Worker capacity** | A compact responsive summary: total worker slots, slots per CPU, estimated worker memory (profile estimate × configured workers, with a 256 MB fallback for each custom/drifted pool), the separate absolute PHP memory-limit ceiling, host RAM, and a healthy/warning/critical status. Missing host capacity or unreadable/malformed pool data is shown as unknown rather than healthy. Estimates are planning metadata, not guaranteed usage: ondemand workers are not permanently resident. |
