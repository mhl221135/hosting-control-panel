...
if git grep -I -n -E 'cfat_[A-Za-z0-9_-]{20,}|cfut_[A-Za-z0-9_-]{20,}|-----BEGIN ([A-Z ]+)?PRIVATE KEY-----' -- . >/tmp/hosting-security-patterns.$$ 2>/dev/null; then
  printf 'Potential token or private key material found in tracked files:\n' >&2
  cut -d: -f1-2 /tmp/hosting-security-patterns.$$ >&2
  failed=true
fi
rm -f /tmp/hosting-security-patterns.$$

deny_patterns="${HOSTING_SECURITY_DENY_PATTERNS:-$project_dir/.security-deny-patterns}"
if [ -s "$deny_patterns" ]; then
  if git grep -I -n -i -f "$deny_patterns" -- . >/tmp/hosting-security-domains.$$ 2>/dev/null; then
    printf 'A locally denied production identifier was found in tracked text:\n' >&2
    cut -d: -f1-2 /tmp/hosting-security-domains.$$ >&2
    rm -f /tmp/hosting-security-domains.$$
    failed=true
  fi
fi
